...
Class PrdReqClass{
	...
	function readPrdReq($conn_id, $prdReq, $pi_no, $po_no){
	...
	//	$sql = $sql . ", if(a.sndmail_seq is null, DATE_FORMAT(a.cnfm_dt, '%y%m-%d'), concat(DATE_FORMAT(a.cnfm_dt, '%y%m-%d'), concat('-',a.sndmail_seq))) txt_swm_no";
		...
		$result = mysqli_query($conn_id,  $sql ) or die("Couldn t execute query.".mysql_error($conn_id));
	...
	#	echo $sql_dtl;
		...
		$result2 = mysqli_query($conn_id,  $sql_dtl ) or die("Couldn t execute query.".mysql_error($conn_id));
